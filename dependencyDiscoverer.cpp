...
private:
	...
	std::mutex mutex;
public:
	void insert_safe(std::string key, std::list<std::string> value){
		std::unique_lock<std::mutex> lock(mutex);
		map.insert({key, value});
	}

	std::unordered_map<std::string, std::list<std::string>>::iterator find_safe(std::string key){
		std::unique_lock<std::mutex> lock(mutex);
		return map.find(key);
	}
	...
	std::unordered_map<std::string, std::list<std::string>>::iterator end() {return map.end();}
};
...
public:
	...
	std::string front() {return list.front();};
};

std::vector<std::string> dirs;
auto theTable = unordered_map{};
auto workQ = list{};
...
static void process(const char *file, std::list<std::string> *ll) {
  ...
  while (fgets(buf, sizeof(buf), fd) != NULL) {
    ...
    *q = '\0';
    ...
    if (theTable.find_safe(name) != theTable.end()) { continue; }
    // ... insert mapping from file name to empty list in table ...
    theTable.insert_safe(name, {});
    // ... append file name to workQ
    workQ.append( name );
  }
  // 3. close file
...
int main(int argc, char *argv[]) {
  //retCode for termination within threads and threadNumber from CRAWLER_THREADS
  int retCode = 0;
  int threadNumber = 2;
   // 1. look up CPATH in environment
  char *cpath = getenv("CPATH");
  
  //Acces CRAWLER_THREADS and save it if it exists
  char *crawler = getenv("CRAWLER_THREADS");
  if (crawler){
    threadNumber = atoi(crawler);
  }
  ...
  for (i = start; i < argc; i++) {
    ...
    theTable.insert_safe(obj, { argv[i] });
    
    // 3b. insert mapping from file.ext to empty list
    theTable.insert_safe(argv[i], { });
    
    // 3c. append file.ext on workQ
    workQ.append( argv[i] );
    }

  //Create vector of worker threads
  std::vector<std::thread> workerThreads;
  for (int j = 0; j < threadNumber; j++){
    workerThreads.push_back(std::thread([retCode]()mutable{
    ...
    while ( workQ.size() > 0 ) {
      ...
      if (theTable.find_safe(filename) == theTable.end()) {
  ...
  //join each thread in the workerThreads vector
  std::for_each(workerThreads.begin(), workerThreads.end(), [](std::thread &t)
  {
    t.join();
  });
